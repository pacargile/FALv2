...
class ReadKurucz(object):
    ...
    def readspecbin(self,specbinpath):
        ...
        NELIONi   = np.ones(rsb_in['nlines'],dtype=np.int32)
        Ei        = np.ones(rsb_in['nlines'],dtype=np.float64)
        EPi       = np.ones(rsb_in['nlines'],dtype=np.float64)
        NBLOi     = np.ones(rsb_in['nlines'],dtype=np.int32)
        NBUPi     = np.ones(rsb_in['nlines'],dtype=np.int32)
        ISO1i     = np.ones(rsb_in['nlines'],dtype=np.int32)
        ISO2i     = np.ones(rsb_in['nlines'],dtype=np.int32)
        X1i       = np.ones(rsb_in['nlines'],dtype=np.float32)
        X2i       = np.ones(rsb_in['nlines'],dtype=np.float32)
        XJi       = np.ones(rsb_in['nlines'],dtype=np.float32)
        XJPi      = np.ones(rsb_in['nlines'],dtype=np.float32)
